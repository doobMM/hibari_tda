...
- **SoundFont 경로는 사용자 로컬** — `C:/soundfonts/UprightPianoKW-...`는 다른 환경에서 동작 안 함. 배포 시 환경변수 `SF2_PATH` 권장.
- **FluidSynth 의존** — `pip install pyfluidsynth` 필요, 일부 Windows 환경에서 libfluidsynth DLL 수동 설치 필요.
- Reverb/Chorus 파라미터는 **사용자 선호 고정** (`feedback_piano_rendering.md`). 값 변경 금지.
- MusicXML 변환은 **music21의 MuseScore 의존** 없이 pretty_midi 경로가 안정적. `gen_harmony_wav.musicxml_to_piano_wav` 먼저 시도.
- 30-60초 소요 — background=true 또는 progress 주기적 보고.
- 출력 WAV 피크 정규화 0.9 — 클리핑 방지. 연속 재생 시 loudness 매칭 필요하면 별도 EBU R128 측정.

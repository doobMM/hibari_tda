...
| 52 | **A** | §6 마디 단위 생성 α=0.25 per-cycle τ 재탐색 | **Task 56 선행** | 마디 best global 0.01479 기록을 α=0.25로 초과 가능? 긴 작업. **주의**: Task 51에서 Algo2는 α=0.25 비유의였으므로, 마디 Algo1도 negative 가능성 있음. **Task 56 P3_local 재설계 이후로 연기** (현행 JS 수치가 버그 창 기반이므로 baseline 재산정 필요) |
...
| 56 | **B+D** | §6 P3_local window 재설계 (Option B) — `[32(m+1), 32(m+2))` both instruments | 독립 (선행: Task 38a 수치 재계산) | **배경**: 현행 `t2_lo = t1_lo + 33`은 inst2 창이 inst1 창과 sequential(1-step gap) + inter lag=1이 physical 34-step lag을 참조. 사용자 원설계 의도는 index lag=physical lag(seq_a/seq_b 동기). **작업**: (1) B — `run_phase3_task38a_dft_gap0.py` L355~404 window 수정 + start_modules 31~32개로 재산정 + N=10~20 재실험 (§6.5 P3_local/+C, §6.7 start_module 후보, §6.8 barcode Wasserstein). (2) D — §6.1/§6.2/§6.5/§6.6/§6.7/§6.8 재서술. **용어 재정책**: P3_local 계열 "모듈 기반" → "마디 기반" (생성 단계 "모듈" 표현은 유지). `memory/project_p3local_design_findings_0419.md` |

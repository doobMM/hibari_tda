...
### ⛔ 상호작용 페이지를 만들기 전에 — 6번 실패에서 뽑은 규칙 (2026-09-08)

Hashimoto 의 래칫 원리(harness engineering): **실패하면 증상이 아니라 실패 계급을 찾아
가장 강한 층에 고쳐 박는다.** 아래는 실제로 6번 거절당한 뒤 뽑은 것이다.

**거절 이력**
| 판 | 무엇 | 사용자 판정 |
|---|---|---|
| 1 | 제출 버튼 + 텍스트 힌트 | "이건 게임이 아니야, 즐겁지가 않잖아" |
| 2 | 그림자 맞추기 (α 축) | α 가 죽어 있어 미터가 100% 고정 |
| 3 | 높낮이/밀도 축 | "조작할 때마다 처음으로 돌아가는 게 목표에 어긋나" |
| 4 | arena 파종장 (Elo 투표) | "방금 그 게임 재미없어" |
| 5 | phase.html (주기 손잡이) | "상당히 비직관적이야. **만지고 싶지도 않아**" |
| 6 | motif.html (모티브→고리) | "여전히 직관적이지 않아, 재미없어" |

**실패 계급 (1~3 과 4~6 이 다르다)**
- 1~3 은 **기계적 고장** — 반응 없음/죽은 축/리셋. 전부 고쳤다.
- 4~6 은 **같은 병**이다: **목표도 소유감도 없는 "샌드박스 + 설명"**.
  화면이 *무엇을 하는지*는 말해 주지만 *왜 계속하는지*는 답하지 않았다.
  손잡이를 주고 그 뜻을 글로 설명하는 물건은 전부 거절당했다.

**가장 강한 층의 규칙 — 단추가 아니라 되먹임 회로 (2026-09-08 사용자 지시)**
Raworth, 『도넛 경제학』 여는 글:
> "경제를 무슨 단추나 레버 몇 개로 얼마든지 통제할 수 있는 기계로 보고 그 단추를
>  찾아 헤매고 다니는 짓은 그만둘 때다. 대신 경제를 영속적으로 진화하는 일종의
>  복잡계로 보아 돌보고 관리해야 한다."

위 여섯 판이 전부 **단추와 레버**였다. 그래서 앞으로 상호작용 페이지는
**되먹임 회로 한 쌍**으로 짓는다 — 강화 하나, 균형 하나.

- **강화만 넣으면 사용자 입력이 지워진다.** echo.html 에서 대조군으로 확인했다:
  균형 회로(잔향의 세대 감쇠)를 빼면 점 2개나 8개나 똑같이 고리 7.5 에 고정되고
  매 스텝 2음으로 포화한다. 무엇을 하든 결과가 같으면 통제감은 0 이다.
- **균형이 있으면 "돌봄"이 동력이 된다.** 점 2개는 2분 뒤 사그라지고(1.9→0.3),
  4개는 유지되고(4.0→2.1), 8개는 자란다(4.5→5.9). 방치하면 꺼지니 다시 온다.
- 착수 전 물을 것: **"이 화면에 강화 회로와 균형 회로가 각각 무엇인가?"**
  둘 중 하나라도 이름을 댈 수 없으면 그건 또 단추다.

**용어를 숨기는 것과 메커니즘을 빼는 것은 다르다 (2026-09-08)**
echo.html 첫 판은 위상수학 용어를 안 쓴 대신 **알고리즘까지 뺐다.**
om_bank.json 에서 `cycles` 만 꺼내 룩업표로 쓰고, 같은 파일의 `om_bits` 도
바로 옆의 `generation-algo1.js` 도 부르지 않았다. 각 점은 독립 메트로놈이었다.
- **검사법**: 연구 데이터를 난수로 바꿔 보라. 귀로 구별이 안 되면 그건 우리 연구가 아니다.
- 용어는 **느낀 뒤에** 붙인다. 느끼기 전에 설명하면 판 4~6 이 된다.

**착수 전 통과해야 할 센서 (값싸다 — 코드 쓰기 전에 답한다)**
1. **"왜 내일 또 오나?"에 한 문장으로 답할 수 있나.**
   답이 *"설정을 탐색할 수 있으니까" · "구조를 볼 수 있으니까"* 면 **실패다. 짓지 마라.**
   답은 *"그냥 하면 기분이 좋으니까" · "내가 만든 게 남으니까"* 여야 한다.
2. **첫 조작까지 읽어야 할 글자가 몇 자인가.** 20자를 넘으면 다시 설계한다.
3. **조작 → 소리까지 몇 ms 인가.** 다음 루프를 기다려야 하면 그건 즉각이 아니다.
   (`PianoPlayer.play()` 가 매번 `stop()` 을 부르므로 **탭 전용 플레이어를 따로 둔다.**)
4. **손잡이 하나를 설명하는 데 한 문장이 필요하면 그 손잡이는 뺀다.**

**놓치고 있던 가장 큰 단서**
사용자가 이 프로젝트에서 **유일하게 긍정한 산출물은 음악 자체**다 —
*"초기 생성곡들도 퀄리티가 나쁘지 않아, 템포가 느리기 때문이야.
hibari 원곡과 분위기가 비슷해서 영화 괴물에 어울릴만한 곡들이야."*
인터페이스가 아니다. → **인터페이스를 재미있게 만들려 하지 말고,
좋은 소리가 즉시 나게 하고 인터페이스는 사라지게 한다.**

### 최소 코드 원칙 → `.claude/rules/ponytail.md`

2026-08-15 채택. 사다리(YAGNI → 재사용 → stdlib → 설치된 의존성 → 한 줄 → 최소 구현),
추가보다 삭제, 요청하지 않은 추상화 금지, 버그는 근본 원인을 grep 해서 한 번에.
⚠ **예외**: `experiments/run_*.py` 의 사전 예측·대조군·한계 서술·원본 JSON 저장은 줄이지 않는다.
**코드는 최소로, 연구 기록은 충분히.**
...
### ⛔ 문서 유지보수 종료 (2026-08-15 사용자 결정)

**`docs/academic_paper_full.md` · `docs/환대_포트폴리오.md` 는 더 이상 갱신하지 않는다.**
LaTeX 3파일도 폐기(T57). 새 수치를 논문에 반영하려 하지 말 것 — 토큰 낭비다.

→ **이후 결과의 기록처는 세 곳이다:**
  1. `docs/step3_data/*.json` — 원본 수치 (원본 대조의 기준)
  2. **커밋 메시지** — 설계·대조군·한계·판정을 여기에 쓴다
  3. `memory/` — 세션 간 인계

두 문서에 남은 수치는 2026-08-15(커밋 3489d26) 시점 기준이며 이후 실험은 반영돼 있지 않다.

### 논문–코드 정합성 규칙 (⚠ 위 종료 결정으로 대부분 비활성)

아래는 논문을 수정할 때의 규칙이었다. 문서 유지보수를 하지 않으므로 1·2번은 더 이상
적용되지 않는다. **3번(수치는 JSON 원본 대조)만 유효하며, 커밋 메시지·memory 에 그대로 적용된다.**
...
| ~~**D. 보고서**~~ | ⛔ **폐지 (2026-08-15)** — 논문 md/LaTeX 유지보수 종료. 결과는 JSON+커밋메시지+memory 에 남긴다 | — | — |
...
| 57 | ⛔ **폐기** | ~~LaTeX 3파일 동기화~~ | **2026-08-15 사용자 결정 — 앞으로 하지 않는다.** LaTeX 3파일(`hibari_tda.tex` / `ko.tex` / `report.tex`)은 md 와 동기화하지 않는다. **2026-08-15 추가: md 2종도 함께 폐기.** 어떤 논문 문서도 갱신하지 않는다 |
...
| T5 | ⛔ 보류 | ~~교집합 추출에 곡빈도+온도 반영~~ **청취에서 거부됨 (2026-08-15)** | 음고 JS −6.2%(`freq_intersect`, Welch p=0.0153 · 비교 3개 Holm 후 **p=0.0305**, d=0.65)였으나 **대가로 협화도 −0.0101**(0.6722→0.6620, Welch **p=0.048**)이었다. ⚠ **2026-09-06 정정** — 종전 기재 "Holm 후 p=0.046 / 협화도 −0.011 p=0.024" 는 `qmc_sampling_results.json` 에서 **재현되지 않는다**. 그 JSON 은 유의성 검정을 저장하지 않았고(평균·표준편차·n=30 만 있다), 거기서 재계산하면 위 값이 나온다. −0.011 은 `freq_intersect` 가 아니라 `combined` 조건의 값(−0.0112)으로 보인다. 블라인드 A/B **P2 에서 baseline(균일추출)이 선택**됐다 — 양쪽 동일 OM 이라 공정한 쌍. N=1 이라 결정적이진 않으나 지표 이득과 청각 손실이 같은 크기이므로 **정본 채택 보류**. `run_qmc_sampling.py` |

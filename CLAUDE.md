...
| T4 | A | **τ-leaping onset 생성** — OM 이 스텝별 음 개수를 정하게 | 원곡 리듬분포 JS −24.9%(p=2e-68), OM 지시 추종 r −0.21→**+0.73**(p=0.041). 대가: 음고 JS +22.7%. `run_gillespie_onsets.py` |
| T5 | A | **교집합 추출에 곡빈도+온도 반영** | 음고 JS **−7.9%**(p=0.007, d=0.72). 튜닝된 T=3.0 이 노트결정의 19%에만 걸려 있었음. `run_qmc_sampling.py` |

...
#%%
# birth (or mean of birth and death ) list 잘 추출하면 함수화할 수 있을 듯.
...
>> filepath = '/MATLAB Drive/distance_matrix.mat';

>> load(filepath); % 'distances' 변수가 workspace에 로드됨

>> distances

format long % 숫자 형식 설정 (이것만 필요!)

>>  max_filtration_value = 0.013888888888888888;
stream = api.Plex4.createExplicitSimplexStream();
% 2. Vertex 추가
num_vertices = size(distances, 1);
for i = 1:num_vertices
    stream.addVertex(i-1); % JavaPlex index는 0부터 시작
end

% 3. Edge 추가 (Distance 조건을 만족하는 경우)
for i = 1:num_vertices
    for j = i+1:num_vertices
        if distances(i, j) <= max_filtration_value
            stream.addElement([i-1, j-1], distances(i, j)); % Edge와 filtration 값 추가
        end
    end
end

% 4. 2-Simplex 추가 (Edge들을 포함하고 Distance 조건을 만족하는 경우)
for i = 1:num_vertices
    for j = i+1:num_vertices
        for k = j+1:num_vertices
            % 모든 Edge가 존재하고, 최대 Edge 길이가 filtration 값 이하인 경우
            if distances(i, j) <= max_filtration_value && ...
               distances(i, k) <= max_filtration_value && ...
               distances(j, k) <= max_filtration_value
                
                % 최대 Edge 길이를 filtration 값으로 사용
                filtration_value = max([distances(i, j), distances(i, k), distances(j, k)]);
                stream.addElement([i-1, j-1, k-1], filtration_value); % 2-Simplex와 filtration 값 추가
            end
        end
    end
end

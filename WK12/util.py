...
def is_distance_matrix_from(weight_mtrx, transform_dict, out_of_reach, power : int, refine = True) : 
    """
...
def get_outta_reach(inter_weight, power) :

    step = 10**power
    outta_reach = 1 + 2 * 1 / (inter_weight[inter_weight!=0].min().min() * step)

    return outta_reach

def search_timeflow_cycles(intra_weights : pd.DataFrame, adn_1_chord_i : list, adn_2_chord_i : list, 
                           inter_lag : int, refine_dict : dict, output_dir : str,
                            rate_start = 0.00, rate_end = 1.5, power = -2,
                            loglog : bool = True) :
    """  
    Args :
    ...
    inter_weight = get_chords_inter_connected(adn_1_chord_i, adn_2_chord_i, lag=inter_lag)
    ...
    plot_lifespan_results(cycles_info, refine_dict = refine_dict, inter_lag = inter_lag, power = power, 
                          rate_start = rate_start, rate_end = rate_end, loglog=loglog, output_dir = output_dir)
...
def get_scattered_cycles_df(df : pd.DataFrame, cycle_labeled : dict, binary : bool = True):
    """
...
def merge_sequential_cycle_rBD(dict1, dict2):
    """
...
def plot_lifespan_results(lifespan_results, refine_dict : dict, inter_lag : int, power : int,
                          rate_start : float = 0.0, rate_end : float = 1.5,
                          loglog=True, figsize_ = (6,4), output_dir : str | None = "./power_search"):
  """
  ...
  date = get_now("%Y%m%d")
  title_string = f'{date}_h1_t_{title_prefix}_1e{power}_{rate_start}_{rate_end}_'
...
def label_cycle(cycle_persistence : dict, transform_dict : dict,
    ...
    def custom_sort_key(cycle):
        return (cycle[0], cycle[1], cycle[2])  # 첫 번째, 두 번째 요소를 기준으로 정렬
...
def cycle_rBD_to_pkl(cycle_persistence : dict, refine_dict : dict, inter_lag : int, 
                     rate_start : float | None, rate_end : float | None, power : int,
                     output_dir : str = "./pickle"):
  """
  딕셔너리 형태의 데이터를 Pandas dictFrame으로 변환합니다.

  Args:
    cycle_persistence: cycle을 구성하는 vertex label들을 key로, 
          rate, birth, death 튜플의 리스트를 value로 가지는 딕셔너리.
          get_rBD_groupedBy_cycle의 리턴값
...
#   df.to_csv(f'cycle_rBD_t_{name}{inter_lag}_1e{power}.csv', index=False) # , float_format='%.20f'
  if (rate_start is None) and (rate_end is None) : 
    filename = f'cycle_rBD_t_{name}{inter_lag}_1e{power}.pkl'
  ...
  elif (rate_start is not None) and (rate_end is not None) :
    filename = f'cycle_rBD_t_{name}{inter_lag}_1e{power}_{rate_start}~{rate_end}.pkl'
    filepath = os.path.join(output_dir, filename)  # 폴더 경로와 파일 이름 결합
    df.to_pickle(filepath)  # pickle로 저장
    print(f"Pickle 파일 '{filename}'가 성공적으로 생성되었습니다.")

  else :
    print("rate_start, rate_end 둘 다 입력하거나 둘 다 안한 버전만 파일명에 반영할 수 있습니다.")

  return df

def cycle_rBD_from_pkl(pkl_file, dir="./pickle"):
    """
...
def plot_cycle_BirthDeath_over_rate(cycle_persistence: dict, subplot_in_a_row : int = 3, power = -2):
    """
...
def search_scale_for_threshold(hibari_notes : pd.DataFrame | None, cycles_strong, cycle_idx, cycle_labeled, threshold : float = 0.8, 
                             get_plot : bool = True, brute_force : bool = False, output_dir = 'scale_evaluation') :


    cycle_scattered = check_cycle_if_plausible(cycles_strong, column_idx = cycle_idx, cycle_labeled = cycle_labeled)
    initial_scale = len(cycle_labeled[0])
    ...
    while True:
        ...
        cons_scattered = access_by_consecutive_indices(cycles_strong, cons_indicies, column_idx=cycle_idx)
        scale_reduction = len(cons_scattered) / len(cycle_scattered)

        # print(f"Scale: {scale}, Scale Reduction: {scale_reduction}")

        if scale_reduction <= min_reduction:
            # print(f"Scale reduction fell below minimum reduction {min_reduction}")
            ...
            # print(f"Scale reduction reached threshold {threshold}. Stopping at scale: {scale}\n")
            break  # threshold 이하로 떨어지면 루프 종료
...
def evaluate_threshold(cycles_strong : pd.DataFrame, cycle_labeled : dict, goal : float, output_dir : str) :
    ...
    for cycle_idx in cycle_labeled.keys():
        ...
        cons_indicies, cycle_onoff, for_score = search_scale_for_threshold(hibari_notes = None, cycles_strong = cycles_strong, 
                                                                        cycle_idx = cycle_idx, cycle_labeled = cycle_labeled, 
                                                                        threshold = goal, output_dir = output_dir)

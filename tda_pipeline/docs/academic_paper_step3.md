...
- __이진 (binary)__: $a_{c,t} = \mathbb{1}[\,V(c)$의 note가 시점 $t$에 하나라도 활성$\,]$. 단순 OR 연산.
- __연속값 (continuous)__: $a_{c,t} = \dfrac{\sum_{n \in V(c)} w(n) \cdot \mathbb{1}[n \in A_t]}{\sum_{n \in V(c)} w(n)}$, 여기서 $w(n) = 1 / N_{\text{cyc}}(n)$은 note $n$의 **희귀도 가중치** (적은 cycle에 등장하는 희귀 note일수록 높은 가중치). 즉 cycle을 구성하는 note 중 *얼마나 많은 비율이* 활성화되어 있는지를 0~1 실수로 표현한다.

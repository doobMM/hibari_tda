...
module and replicating it across the full timeline. Here, a module is a
32-step (four-bar) repeating melodic unit with an A-B-A'-C form, repeated
33 times in instrument~1. Under DFT $\alpha=0.25$, module PH yields $K=14$
cycles (prototype OM: $32\times14$). In a Section~7.2 rerun, we also
reinterpret the full 1088 rows as $34\times32$ blocks (``bar'' there means
a computational 32-step block). We first compare four prototype strategies:
\begin{itemize}[nosep]
\item \textbf{P0}: first-block copy.
\item \textbf{P1}: OR over 34 blocks.
\item \textbf{P2}: majority vote over 34 blocks.
\item \textbf{P3\_local}: recomputed local PH.
\end{itemize}
...
Exhaustive start-module search over all 33 modules (inst~1 basis,
start~module $0\ldots32$) shows that start~0 is not uniquely best in DFT
mode (rank~5 by mean JS). Here $k=10$ is the best-of-$k$ candidate count
within each seed, and $N=10$ is the number of independent seeds per start
module. The global best trial is start~module~1, seed~9309, JS~$=0.01479$,
essentially matching the
full-song Phase~1 per-cycle best ($0.01489$); Phase~2 record ($0.01156$) is $+28.0\%$ ahead.
...
\item \textbf{Chord-space mismatch}: module-local PD is computed from
      ...
      complex.
\item \textbf{Asymmetric window-alignment limitation}: module-local PH uses
      both windows (inst1 $[32m,32m+32)$ and inst2 $[32m+33,32m+65)$), but
      their fixed 33-step offset makes the inst2 contribution
      start-dependent, introducing structural bias against direct 1:1
      comparison with full-song PD.
...
\item \textbf{Module-level generation can match full-song quality.}
      ...
      reaches $0.01479$ (matches Phase~1 best $0.01489$; $+28.0\%$ vs Phase~2 record $0.01156$).

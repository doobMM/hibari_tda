...
\subsection{Direction A: Topology-Preserving Note Redistribution}
\label{subsec:dirA}

Cycle vertex sets are replaced with new notes selected to minimize
the total pairwise distance.  For each cycle, we construct a
Tonnetz (or voice-leading) distance matrix between original and
candidate notes, then solve the optimal assignment via the
Hungarian algorithm (\texttt{scipy.optimize.linear\_sum\_assignment}).
The overlap matrix is preserved intact; only the note identities change.

\begin{table}[t]
\centering
\caption{Note redistribution (Algorithm~1, Tonnetz).}
\label{tab:redistrib}
\begin{tabular}{lcccc}
\toprule
Setting & Note err & Cycle err & DTW & $\Delta$DTW \\
\midrule
baseline  & --- & --- & $2.21$ & --- \\
...
With the widest pitch range (MIDI 40--88), DTW increases by $61.4\%$
while cycle distance error remains stable at $\sim 1.1$
(Table~\ref{tab:redistrib})---topological structure is preserved while
melody is genuinely different.

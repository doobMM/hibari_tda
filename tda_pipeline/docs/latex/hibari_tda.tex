...
overlap, per-cycle $\tau_c$ (Phase~1, $\alpha=0.5$, $K=19$) reaches
$0.01489 \pm 0.00143$ ($+58.7\%$, $p=2.48\times10^{-26}$); re-optimizing
at $\alpha=0.25$, $K=14$ (Phase~2) yields the new Algorithm~1 best:
$\mathbf{0.01156 \pm 0.00147}$ ($-22.35\%$, Welch $p=4.94\times10^{-11}$),
confirming $\alpha=0.25$ optimal in both binary-OM and per-cycle $\tau_c$.
...
In bar-level generation (DFT $\alpha=0.25$, $K=14$; Option~B window, 2026-04-19), the best
P3+C trial reaches JS $0.0167$, and exhaustive search over 32 start
markers yields a global best of $0.01479$ (start $m=0$, seed $9309$), essentially matching the
full-song Phase~1 ($\alpha=0.5$) per-cycle result ($0.01489$); Phase~2 record ($\alpha=0.25$, $0.01156$) is $+28.0\%$ ahead.
\end{abstract}
...
\paragraph{Remark on metric axioms}
\label{para:metric_axioms}
Experimental verification shows that Tonnetz and voice-leading satisfy all
metric axioms, while frequency violates identity ($23$ cases) and the triangle
inequality ($1{,}327$ cases; non-metric), and DFT violates identity
($66$ cases; pseudometric---an intentional consequence of its magnitude-only
design).
The classical stability theorem of Cohen-Steiner et al.\ \cite{cohensteiner2007}
(bounding bottleneck distance by $\|d_1-d_2\|_\infty$ for metric inputs)
therefore does not apply directly.
Instead, we invoke Heo et al.\ \cite{heo2025}: assigning $D$ as edge
weights on the complete graph $K_n$ trivially satisfies their
\emph{path-representable} and \emph{cost-dominated} conditions.
By Theorem~3.4 thereof, an order-preserving injection
$\mathrm{bcd}_1(d_i)\hookrightarrow\mathrm{bcd}_1(d_j)$ exists for $d_i\le d_j$,
justifying 1-cycle analysis for all four distances.
(The guarantee does not extend to $k\ge 2$; see Theorem~5.1 of \cite{heo2025}.)
...
\subsection{Bar-Level Generation}
\label{subsec:module}

\emph{hibari}'s 32-step modular structure motivates generating one
bar and replicating it across the full timeline. Here, a \emph{module}
(used as a naming for the repeated melodic arrangement unit) is a
32-step four-bar repeating unit with an A-B-A'-C form, repeated
33 times in instrument~1; the \emph{P3\_local} family recomputes
persistent homology within a single 32-step bar, and we therefore refer
to it as \emph{bar-local PH}. Under DFT $\alpha=0.25$, full-song PH
yields $K=14$ cycles. We first compare four prototype strategies for
constructing the $32\times14$ prototype overlap matrix:
\begin{itemize}[nosep]
\item \textbf{P0}: first-block copy (density $\approx 0.018$).
\item \textbf{P1}: OR over 34 blocks (density $=1.000$).
\item \textbf{P2}: exclusive (AND) over 34 blocks (density $\approx 0$).
\item \textbf{P3\_local}: recomputed bar-local PH on the common window
  $[32(m+1),32(m+2))$ for both instruments (2026-04-19 Option~B design;
  index lag equals physical lag).
\end{itemize}

\begin{table}[t]
\centering
\caption{Prototype strategy comparison under Option~B (DFT $\alpha=0.25$, $N=10$).}
\label{tab:module_proto}
\begin{tabular}{lccc}
\toprule
Strategy & Density & JS (mean$\pm$std) & Best \\
\midrule
P0 (first\_block) & $0.018$ & $0.1040 \pm 0.0219$ & $0.0698$ \\
P1 (OR-34) & $1.000$ & $0.0621 \pm 0.0167$ & $0.0422$ \\
P2 (exclusive, AND) & $0.000$ & $0.0900 \pm 0.0308$ & $0.0515$ \\
\textbf{P3\_local} & \textbf{$0.299$} & \textbf{$0.0474 \pm 0.0187$} & \textbf{$0.0226$} \\
\bottomrule
\end{tabular}
\end{table}

We then evaluate post-processing improvements on top of the bar setting:

\begin{table}[t]
\centering
\caption{Bar-level improvements under Option~B (DFT, baseline full-song
$0.0213 \pm 0.0021$).}
\label{tab:module}
\begin{tabular}{lcccc}
\toprule
...
\midrule
P0 baseline      & $0.1082 \pm 0.0241$ & $0.0701$ & $0.787$ \\
C: best-of-10    & $0.0800 \pm 0.0171$ & $0.0570$ & $0.913$ \\
D: cov$\ge$20/23 & $0.0808 \pm 0.0171$ & $0.0570$ & $0.887$ \\
P3\_local        & $0.0636 \pm 0.0207$ & $0.0335$ & $0.791$ \\
\textbf{P3\_local + C} & $\mathbf{0.0417 \pm 0.0145}$ & $\mathbf{0.0167}$ & $0.900$ \\
\bottomrule
\end{tabular}
\end{table}

Exhaustive start-marker search over 32 markers ($m = 0\ldots31$, analysis
window $[32(m+1),32(m+2))$ shared by both instruments; the original first
bar $[0,32)$ is thus excluded by the Option~B convention) shows that
$m=0$ is not uniquely best in DFT mode (rank $16/32$ by mean JS; the
previously reported rank~5 under the asymmetric window did not survive
Option~B window unification). Here $k=10$ is the best-of-$k$ candidate
count within each seed, and $N=10$ is the number of independent seeds
per start marker. The global best trial is start marker $m=0$, seed~9309,
JS~$=0.01479$, essentially matching the full-song Phase~1 per-cycle best
($0.01489$); Phase~2 record ($0.01156$) is $+28.0\%$ ahead.
...
Per-cycle $\tau_c$ (Phase~1, $\alpha=0.5$) & $0.01489 \pm 0.00143$ & $+58.7\%$ \\
\textbf{Per-cycle $\tau_c$ $\star$ (Phase~2, $\alpha=0.25$)} & $\mathbf{0.01156 \pm 0.00147}$ & $\mathbf{-22.35\%}$ \\
\bottomrule\end{tabular}
\end{table}

Per-cycle $\tau_c$ Phase~1 ($\alpha=0.5$, JS $0.01489$) and Phase~2 ($\alpha=0.25$, JS $\mathbf{0.01156}$)
confirm that cycle-specific activation thresholds matter
(Phase~1 Welch $p = 2.48\times10^{-26}$; Phase~2 $p = 4.94\times10^{-11}$).
...
\paragraph{Remark on Algo\,2 $\alpha$ setting (Task~51).}
After confirming $\alpha=0.25$ optimal for Algorithm~1
(\S\ref{subsec:percycle} and \S\ref{subsec:alpha}), we retested
FC-cont at $\alpha=0.25$ ($K=14$, DFT, gap=0, $N=10$,
\texttt{fc\_cont\_dft\_alpha025\_results.json}).
Result: JS $= 0.00057 \pm 0.00046$ vs.\ $\alpha=0.5$
JS $0.000348$; Welch $p = 0.168$ (not significant).
Reducing $K$ from 19 to 14 decreases the learning signal for FC,
reversing the benefit seen in Algorithm~1.
\textbf{Algorithm~2 best remains $\alpha=0.5$, JS $=0.000348$.}
...
\subsection{Barcode Wasserstein Bar Selection}
\label{subsec:wasserstein}

Under DFT $\alpha=0.25$ with Option~B (common window $[32(m+1),32(m+2))$
for both instruments), we compare bar-local barcodes against the
full-song barcode via Wasserstein distance (32 start markers).
The observed correlation with generation JS is
\textbf{Pearson $r = +0.2457$}, a weak-to-moderate positive correlation.
This is a reversal of the previously reported $r=-0.054$ under the
asymmetric-window design: unifying the two instrument windows restored
the intended design semantics (index lag equals physical lag) and
recovered a positive barcode--JS relation. The correlation is, however,
still weaker than the Tonnetz observation ($r=0.503$): Wasserstein-based
bar ranking remains useful only as a coarse screening step in DFT mode;
final selection should still rely on direct JS evaluation (the
exhaustive search of \S\ref{subsec:module}).

Three caveats remain:
\begin{enumerate}[nosep]
\item \textbf{Correlation strength below moderate}: $|r|=0.246$ is in the
      weak-to-moderate range; the best $W$-trial ($W=0.288$) achieves
      JS $=0.0760$, still far from the global minimum $0.01479$.
\item \textbf{Chord-space mismatch}: bar-local PD is computed from
...
\item \textbf{Continuous-overlap refinement is substantial.}
      Per-cycle $\tau_c$ (Phase~1, $\alpha=0.5$) reaches $0.01489 \pm 0.00143$ ($+58.7\%$);
      Phase~2 ($\alpha=0.25$) re-optimization: $\mathbf{0.01156 \pm 0.00147}$ ($-22.35\%$, $p=4.94\times10^{-11}$),
      and FC-cont reaches
      $0.00035 \pm 0.00015$.

\item \textbf{Bar-level generation can match full-song Phase~1.}
      Under Option~B common window, P3\_local+C gives best trial $0.0167$
      (below full-song $0.0213$); exhaustive 32-marker search reaches
      $0.01479$ at $m=0$, seed $9309$ (matches Phase~1 best $0.01489$;
      $+28.0\%$ vs Phase~2 record $0.01156$).
...
\item \textbf{Complex/simul and Wasserstein are distance-specific.}
      In DFT settings, simul mixing degrades performance. Under the
      earlier asymmetric-window design the barcode--Wasserstein
      correlation collapsed ($r=-0.054$); unifying both instrument
      windows (Option~B) restored it to $r=+0.2457$, still weaker than
      Tonnetz ($r=0.503$). Both therefore require metric-conditioned use.
\end{enumerate}

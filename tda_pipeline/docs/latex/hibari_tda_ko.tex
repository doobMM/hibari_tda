...
DFT 연속 중첩행렬에서 cycle별 임계값 최적화(per-cycle $\tau_c$)를 적용하면
$\mathbf{0.01489 \pm 0.00143}$에 도달하며
(균일 $\tau=0.35$ 대비 $+58.7\%$, Welch $p = 2.48 \times 10^{-26}$),
...
전곡 per-cycle 최적값($0.01489$)과 사실상 동등하다.
\end{abstract}
...
전 타임라인에 복제하는 설정을 실험하였다. DFT $\alpha=0.25$에서
모듈 PH는 $K=14$ cycle(프로토타입 중첩행렬 크기: $32\times14$)을 갖는다.
먼저 네 가지 프로토타입 전략을 비교하였다:
\begin{itemize}[nosep]
\item \textbf{P0}: 첫 모듈 복사
\item \textbf{P1}: 활성 cycle 합집합
\item \textbf{P2}: 모듈 고유 cycle
\item \textbf{P3}: 모듈별 활성 중앙값 기반 선택
\end{itemize}
...
33개 시작 모듈 전수 탐색 결과, DFT 모드에서 모듈~0은 평균 JS 기준
rank~5로 최적이 아니다. 전역 최저 단일 시행은
start~module~1, seed~9309에서 JS~$=0.01479$이며,
전곡 per-cycle 최적값($0.01489$)과 사실상 동일하다.
...
($0.01489 \pm 0.00143$)에 도달한다.
균일 $\tau=0.35$ 대비 Welch $p = 2.48 \times 10^{-26}$로 유의하며,
...
\item \textbf{단일 악기 한계}: 모듈 국소 PH는 단일 악기 시퀀스만 사용하므로,
      큰 JS 개선에 기여하는 악기 간 가중 정보가 누락된다.
...
\item \textbf{연속 중첩행렬 정교화의 효과가 크다.}
      Per-cycle $\tau_c$는
      $0.01489 \pm 0.00143$ (균일 $\tau=0.35$ 대비 $+58.7\%$),
      FC-cont는

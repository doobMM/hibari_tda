...
P3+C 단일 시행 최저 JS가 $0.0167$이고,
32개 시작 마디 전수 탐색의 전역 최저는 $0.01479$ ($m=0$, seed~9309)로,
...
\emph{hibari}의 32-step 모듈 구조를 바탕으로, 단일 마디를 생성하여
전 타임라인에 복제하는 설정을 실험하였다. 여기서 모듈은
32-step(음악적 4마디) 길이의 A-B-A'-C 반복 선율 단위를 가리키는 배치 용어이며,
inst~1에서 33회 반복된다. P3\_local 계열은 대상 32-step 마디 구간에서
persistent homology를 재계산하므로 이를 \emph{마디-local PH}라 부른다.
DFT $\alpha=0.25$에서 전곡 PH는 $K=14$ cycle을 갖는다.
먼저 $32\times14$ 프로토타입 중첩행렬을 구성하는 네 가지 전략을 비교하였다:
\begin{itemize}[nosep]
\item \textbf{P0}: 첫 블록 복사 (density $\approx 0.018$)
\item \textbf{P1}: 34블록 OR 합집합 (density $= 1.000$)
\item \textbf{P2}: 34블록 공통(exclusive, AND; density $\approx 0$)
\item \textbf{P3\_local}: 두 악기 모두 $[32(m+1),32(m+2))$ 동일 창에서
      마디-local PH 재계산 (2026-04-19 Option~B 설계; index lag = physical lag)
\end{itemize}

\begin{table}[t]
\centering
\caption{프로토타입 전략 비교 — Option~B (DFT $\alpha=0.25$, $N=10$).}
...
이후 마디 설정 위에 후처리 개선 전략을 추가 평가하였다:

\begin{table}[t]
\centering
\caption{마디 단위 개선 성능 — Option~B (DFT, 전곡 기준선
$0.0213 \pm 0.0021$).}
\label{tab:module}
\begin{tabular}{lcccc}
\toprule
...
32개 시작 마디 전수 탐색($m=0\ldots31$, 분석 창 $[32(m+1),32(m+2))$은
두 악기가 공유; 원곡 첫 마디 $[0,32)$는 Option~B 창 규약상 탐색 대상에서 제외)
결과, DFT 모드에서 $m=0$은 평균 JS 기준 rank~$16/32$로 특별히 우수하지 않다
(이전 비대칭 창 기반의 rank~5 우위는 Option~B 창 통일 후 소실).
여기서 $k=10$은 seed 내부 best-of-$k$ 후보 수, $N=10$은 시작 마디당
독립 반복 횟수이다. 전역 최저 단일 시행은 $m=0$, seed~9309에서
JS~$=0.01479$이며,
...
Welch $p = 0.168$ (비유의).
$K$ 감소(19→14)로 FC 학습 신호가 오히려 감소하여,
Algorithm~1에서의 이점이 역전된다.
...
DFT $\alpha=0.25$ + Option~B (두 악기 모두 $[32(m+1),32(m+2))$ 동일 창)에서
32개 시작 마디의 국소 barcode와 전곡 barcode의 Wasserstein 거리를 비교한 결과,
생성 JS와의 상관은 \textbf{Pearson $r=+0.2457$}로 약한 양의 상관이 회복되었다.
이는 이전 비대칭 창 설계에서 보고된 $r=-0.054$의 \emph{부호 반전}으로,
두 악기 창을 통일(index lag = physical lag)함으로써 본래 설계 의도가 정량적으로
검증되었음을 의미한다. 상관 강도는 여전히 Tonnetz($r=0.503$)보다 약하므로,
Wasserstein 기반 마디 선택은 DFT 조건에서 \emph{초벌 screening}에는 유용하나
최종 선택에는 JS 직접 평가(§\ref{subsec:module}의 전수 탐색)가 더 신뢰할 수 있다.

세 가지 주의점은 다음과 같다:
\begin{enumerate}[nosep]
\item \textbf{상관 강도는 중등도 이하}: $|r|=0.246$은 weak-to-moderate 범위이며,
      최적 $W$-trial ($W=0.288$)의 JS는 $0.0760$으로 전역 최저 $0.01479$와 격차가 크다.
\item \textbf{화음 공간 불일치}: 32-step 마디의 chord vocabulary는
...
\item \textbf{마디 단위 생성은 전곡 Phase~1 품질에 도달 가능하다.}
      Option~B 동일 창에서 P3\_local+C의 단일 시행 최저는 $0.0167$ (전곡 $0.0213$ 하회),
      32개 시작 마디 전수 탐색 전역 최저는 $m=0$, seed~9309에서 $0.01479$로
      전곡 Phase~1 최저 $0.01489$와 동등 ($\alpha=0.25$ 신기록 $0.01156$ 대비 $+28.0\%$ 열세).

\item \textbf{Complex/simul 및 Wasserstein은 거리함수 의존적이다.}
      DFT에서는 simul 혼합이 성능을 저하시키며,
      barcode-Wasserstein 상관은 비대칭 창 설계에서 $r=-0.054$까지 붕괴했으나

...
또한 DFT continuous OM에서 per-cycle $\tau_c$ 최적화를 적용하면
JS가 $\mathbf{0.01489 \pm 0.00143}$로 낮아지며
(uniform $\tau=0.35$ 대비 $+58.7\%$, Welch
$p = 2.48 \times 10^{-26}$), Algorithm~1 최저 성능을 달성한다.
...
$\mathbf{0.01479}$로 full-song 최저값($0.01489$)과 사실상 동등하다.
...
full-song per-cycle 최저값($0.01489$)과 사실상 동등하다.
...
\textbf{Cycle별 $\tau_c$} & $\mathbf{0.01489 \pm 0.00143}$ & $\mathbf{+58.7\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{연속값 OM 정교화의 효과.}
      per-cycle $\tau_c$ 최적화로
      $0.01489 \pm 0.00143$ (uniform 대비 $+58.7\%$)을 달성했고,
...
\item \textbf{모듈 단위 생성의 full-song 품질 도달.}
      ...
      full-song 최저($0.01489$)와 사실상 동등하다.

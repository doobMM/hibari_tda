...
(P4 — module-local PH from scratch — 은 별도 구현이 더 복잡하여 본 보고에서는 제외하였다. §7.1.4에서 후속 과제로 언급한다.)
...
본 §7.1 구현은 모듈 단위 생성이 원리적으로 가능하나 **module-level randomness가 33× amplify되는 본질적 한계**가 있음을 보였다. 이전 보고서가 채택한 OR (P0) prototype은 사실상 random sampling에 가까웠음이 비교 실험으로 드러났으며, 본 개정판은 보다 selective한 P1 (mean → $\tau = 0.5$) prototype 으로 전환하였다.

P1 의 평균 JS $0.110$ 은 baseline $0.040$ 의 $2.8$배이며, best trial $0.068$ 은 $1.7$배이다. 그러나 prototype 전략 자체로는 추가 개선의 여지가 거의 없으며, 진짜 개선은 위 개선 C/D 와 같이 *randomness 자체를 통제하는 후처리 메커니즘*에서 와야 한다.

__본 연구에서의 의의는 변하지 않는다.__ §7.1 은 본 연구의 "topological seed (Stage 2-3)" 와 "음악적 구조 배치 (Stage 4 arrangement)" 가 서로 직교하는 두 축임을 실증한 첫 사례이다. 단 $1{-}2$ ms 의 모듈 생성 속도는 실시간 인터랙티브 작곡 도구의 가능성을 열어두며, 한 곡의 topology 를 다른 곡의 arrangement 에 이식하는 *topology transplant* 같은 새로운 응용을 가능하게 한다.

### 즉시 가능한 다음 단계

1. __개선 C 구현__: $k = 10$ best-of-$k$ selection 을 P1 위에 추가
2. __개선 D 구현__: note coverage $\ge 20/23$ 강제 후 평균 JS 재측정
3. __개선 P4 구현__: 첫 모듈의 sub-distance-matrix 로 module-local PH 시도
4. __청각적 평가__: best trial (seed 7108) 의 MusicXML 을 오디오로 렌더링하여 원곡과 A/B 청취 비교
...
- `tda_pipeline/run_module_generation_v2.py` — 5가지 prototype 전략 비교 실험
- `tda_pipeline/docs/step3_data/step71_module_results.json` — 본 실험 10 trials 원본 수치
...
- `tda_pipeline/output/step71_module_best_seed7108.musicxml` — 최우수 trial의 생성 결과

...
- **이미 실패/한계 확인 — 중복 제안 금지**:
  ...
  - 음악이론 미적 지표(협화도+성부진행+도약) re-ranking: 합성 지표 자체가 calibration 실패(셔플이 원곡보다 점수 높음). 협화도(C) 성분만 유효, 나머지는 hibari 를 오히려 페널티(`project_aesthetic_rerank_negative_0613`). ※ 당시 원인으로 적은 "2성부 음역 분리"는 2026-08-10 확인 결과 **사실이 아님** — 두 성부 pitch 집합이 동일하다.

...
__캡션.__ hibari의 H$_1$ cycle 전체 48개에 대한 persistence barcode이다. 가로축은 rate parameter $r_t$ (filtration scale), 각 막대는 한 cycle이 살아 있는 $[\mathrm{birth}, \mathrm{death}]$ 구간을 나타낸다. 48개 cycle은 lifespan 기준 내림차순으로 정렬되어 위쪽에 가장 오래 살아남는 cycle이 온다.

색은 두 가지 의미만을 갖는다.
- __빨강__ (총 11개) — 탐색 범위 $[0, 1.5]$를 넘어서까지 살아남는 cycle. 원본 데이터의 death 값이 $1.5$를 초과하는 모든 cycle을 의미하며, 시각화를 위해 막대를 $r_t = 1.5$에서 잘라내고 우측 끝에 $\infty$ 기호를 표시하였다. 이들은 해당 filtration 범위 안에서는 "구조적으로 안정된" cycle이다.
- __파랑__ (총 37개) — 탐색 범위 안에서 birth와 death가 모두 일어나는 유한 cycle.

위쪽 11개(빨강)는 lifespan이 매우 길고, 그 아래 37개(파랑)는 lifespan이 짧아 거의 한 점처럼 보이는 경우가 많다. 이 두 그룹의 명확한 분리 자체가 hibari의 위상 구조에 "오래 살아남는 핵심 cycle 11개"가 존재한다는 것을 시사한다 (2.3절의 Elder Rule과 연결).
...
__캡션.__ 두 panel은 hibari의 두 번째 악기(inst 2)가 "32 timesteps 단위의 모듈"로 어떻게 구조화되어 있는지를 보여준다.

- __(a) 전체 곡__ — inst 2의 전체 piano roll. 옅은 초록 배경은 한 모듈 영역(32 timesteps)을 나타내며, 짝수/홀수 모듈을 약간 다른 명도로 표시하여 모듈 경계가 시각적으로 드러나도록 하였다. 진한 초록 수직선은 모듈 경계이며, 흰색 영역은 그 시점에 어떤 음도 발성되지 않는 **쉼표** 구간이다. 위쪽 라벨 `M1, M6, M11, ...`은 5개 간격으로 표시된 모듈 번호이다. 곡 전체에 걸쳐 33개의 모듈이 같은 길이로 반복되며, 각 모듈 안에 일정 비율의 쉼표가 있음을 확인할 수 있다.
- __(b) Modules 5 – 6 확대__ — 두 개의 모듈 ($t \in [128, 192)$)을 확대하여 그 안의 실제 음 배치를 자세히 본다. 빨간 사각형이 각 음표(시작 시점, 길이, pitch), 흰색이 쉼표, 옅은 초록 배경이 모듈 영역이다. 이 확대 뷰에서, 각 모듈이 단순한 반복이 아니라 모듈 내부에 자체적인 작은 패턴(작은 상행/하행, 짧은 도약, 고정 음의 반복 등)을 가지고 있음을 직접 관찰할 수 있다.

이 figure가 본 연구에서 중요한 이유는, 본 연구가 음악 생성을 수행할 때 사용하는 chord-height 패턴이 임의의 32-element 수열의 단순 반복이 아니라 **이 모듈 구조 자체**에서 추출된 것이기 때문이다. 즉 위상학적으로 모은 cycle 정보(중첩행렬)와 음악적 모듈 구조가 같은 시간축 위에서 결합되어 음악이 생성되며, Figure 7은 그 음악적 골격을 시각적으로 드러낸다.

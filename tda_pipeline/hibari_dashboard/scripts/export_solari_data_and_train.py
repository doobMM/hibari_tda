"""
export_solari_data_and_train.py — solari 대시보드 데이터 + Transformer 모델 생성
==================================================================================

hibari 외에 solari 곡을 대시보드에서 사용할 수 있도록
(1) data_solari/ 하위 JSON 5종 + MIDI 복사
(2) public/models/transformer_solari.onnx + transformer_solari_meta.json
을 생성한다.

연구 근거:
  - solari는 chromatic(12 PC), voice_leading 거리 + Transformer 모델이 최적.
  - CLAUDE.md §핵심 발견 — 곡의 성격이 최적 도구를 결정한다 참조.

출력 스키마:
  data_solari/ — export_hibari_data.py와 동일한 JSON 필드 구조
...
DATA_SOL = DASH_ROOT / 'data_solari'
...
MIDI_SRC = TDA_ROOT / 'ryuichi-sakamoto-solari.mid'

# ─── 전처리 설정 (run_solari.py와 동일 파라미터) ──────────────────────────
...
OPTIMAL_CONFIG = {
    ...
    'min_onset_gap': 0,
    'threshold': THRESHOLD,
    'description': 'solari 최적: voice_leading 거리 (chromatic 12 PC, 선율적 진행)',
}

# ─── 전처리 (run_solari.preprocess 에서 발췌) ────────────────────────────────

def preprocess(midi_path: str):
    """solari MIDI → notes_label, notes_counts, adn_i, N, T, num_chords 반환."""
    ...
    # 통합 chord map (run_solari.py와 동일)
...
# ─── PH + overlap 계산 (run_solari.compute_ph에서 발췌, voice_leading 고정) ─

def compute_overlap(data):
    """voice_leading 거리로 PH 계산 → (cycle_labeled, binary_ov, cont_act) 반환."""
    from weights import (
    ...
    print(f"  [voice_leading] note 거리 행렬 계산...")
    ...
    # activation matrix 구성 (run_solari.py 방식)
...
def export_overlap_matrices(T, K, binary_ov, cont_act):
    ...
    ref_payload = {
        ...
        'description': (
            'solari voice_leading PH (α=0.5, timeflow, threshold=0.35) 이진 overlap. '
...
# ─── Traversal 헬퍼 (add_cycle_traversal.py와 동일 로직, solari 경로 대응) ──
...
def export_cycles_metadata(cycle_labeled, profile_bd=None):
    ...
    payload = {
        ...
        'source': f'voice_leading timeflow PH (α={ALPHA}, threshold={THRESHOLD})',
        'cycles': cycles_info,
        'description': (
...
def copy_midi(src: Path):
    ...
    if not src.exists():
        ...
        return
    dst = DATA_SOL / 'original_solari.mid'
...
def export_manifest(T, K, N):
    ...
    manifest = {
        ...
        'notes': (
            'solari 최적 설정: voice_leading 거리 + Transformer 모델. '
            f'N={N} notes (tie 정규화), K={K} cycles, T={T} time steps. '
            'Algo2 모델: transformer_solari.onnx (public/models/).'
        ),
    }
...
def save_transformer_meta(K: int, N: int, max_len: int,
    ...
    """transformer_solari_meta.json 저장 (train_fc_and_export.py meta 스키마 + model_type)."""
...
def main():
    ...
    print("=" * 70)
    print("  solari 대시보드 데이터 Export + Transformer 학습")
    print("  최적 설정: voice_leading 거리 + Transformer (chromatic, 선율)")
    print("=" * 70)
    ...
    # ── 2. PH + overlap ───────────────────────────────────────────────────
    print("\n[2] Persistent Homology + overlap 계산 (voice_leading)")
    ...
    # ── 3. data_solari/ JSON export ───────────────────────────────────────
    print("\n[3] data_solari/ JSON 저장")
    ...
    onnx_path = MODELS_DIR / 'transformer_solari.onnx'
    ...
    # ── 8. 메타데이터 저장 ────────────────────────────────────────────────
    print("\n[8] 메타데이터 저장")
    meta_path = MODELS_DIR / 'transformer_solari_meta.json'
    ...
    for p in sorted(DATA_SOL.iterdir()):
        sz = p.stat().st_size
        print(f"  data_solari/{p.name:45s} {sz/1024:8.1f} KB")
    ...
    print(f"    data_solari 경로: hibari_dashboard/data_solari/")
    print(f"    모델 경로: public/models/transformer_solari.onnx")
    print(f"    메타 경로: public/models/transformer_solari_meta.json")
    print(f"    T={T}, K={K}, N={N}")

"""
방향 A: tonnetz_vwide (tonnetz, pitch 40-88) → Algo1 생성 → Piano WAV
"""
...
print("[3/5] tonnetz_vwide 새 note 탐색...")
result = find_new_notes(
    data['notes_label'], cl,
    note_metric='tonnetz', cycle_metric='tonnetz',
    pitch_range=(40, 88), n_candidates=1000, seed=SEED
)
...
out_dir = os.path.join("output", "note_reassign_tonnetz_vwide")
os.makedirs(out_dir, exist_ok=True)

score = notes_to_xml([gen], tempo_bpm=66,
                     file_name="tonnetz_vwide", output_dir=out_dir)

if score:
    mid_path = os.path.join(out_dir, "tonnetz_vwide.mid")
    wav_path = os.path.join(out_dir, "tonnetz_vwide_piano.wav")
